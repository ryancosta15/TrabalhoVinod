...
argumentos = argumentos.parse_argumentos()
...
try:
    with open(argumentos.script, encoding='utf-8') as f:
        script = f.readlines()
except Exception:
...
for c, linha in enumerate(script):
    ...
    if '::info' in linha:       # Ignora comentários
        linha = linha.split('::info', 1)[0]
    if not linha.strip():       # Checa se é uma linha vazia. Se sim, apenas pula para a próxima.
        continue
    dlinha = linha.split(' ')
    ...
    elif len(dlinha) > 2 and dlinha[0] == 'deletar' and dlinha[1] == 'variável':
        monarca.variavel('del', dlinha[2])

    # Verifica se o usuário quer clonar o valor de uma variável
    elif len(dlinha) > 4 and dlinha[0] == 'clonar' and dlinha[1] == 'variável':
        monarca.clonar_valor(dlinha[2], dlinha[4])

    # Verifica se o usuário quer mostrar uma mensagem na tela
    elif len(dlinha) > 3 and (
        ' '.join(dlinha[:3]) == 'mostrar na tela:' or
        ' '.join(dlinha[:3]) == 'mostrar na tela'
    ):
    ...
    # Entrega um erro caso o usuário não digite nenhum comando conhecido pelo Monarca. Será ignorado caso seja uma linha vazia. 
        ...
        monarca.erro('Sintaxe inválida. Consulte a documentação.')
